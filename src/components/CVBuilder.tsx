...
export default function CVBuilder({
  ...
  const handleApplyEditingChanges = () => {
    ...
    const existsIndex = updated.findIndex(c => c.id === editingCv.id);
    const saved = { ...editingCv, lastUpdated: new Date().toISOString() };
    if (existsIndex >= 0) {
    ...
    setActivePolishedCV(saved);
    setActiveBuilderTab(currentUser ? "dashboard" : "create");
  };
