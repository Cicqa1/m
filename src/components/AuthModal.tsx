...
import { apiSaveUser } from "../lib/api";
...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }: AuthModalProps) {
  ...
  useEffect(() => {
    const handleOAuthMessage = (event: MessageEvent) => {
      // Validate origin is from standard run.app, replit.dev, or localhost
  ...
  const handleSignInSuccess = async (profile: { id: string; email: string; fullName: string; picture?: string; createdAt: string }) => {
    ...
    // Use the Google profile ID directly as the stable user identifier
    const finalId = profile.id;
  ...
  const handleNativeSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "signup") {
      ...
      localStorage.setItem("firststep_active_user", JSON.stringify(matchedUser));
      apiSaveUser(matchedUser).catch(() => {});
